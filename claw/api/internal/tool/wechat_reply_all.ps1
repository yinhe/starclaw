# wechat_reply_all.ps1 — One-shot: scan all unread badges, AI vision reply, send
# Injected by Go: {{API_URL}}, {{JWT_TOKEN}}, {{AGENT_ID}}, {{MCP_URL}}
# Returns JSON array of actions taken.
...
function Get-WxRect {
    $fg = [WxRA]::GetForegroundWindow()
    $r = New-Object WxRA+RECT
    [WxRA]::GetWindowRect($fg, [ref]$r) | Out-Null
    return $r
}
...
if (-not (Activate-WeChat)) {
    Write-Output '{"status":"error","message":"failed to activate wechat","replies":[]}'
    exit
}

$wr = Get-WxRect
$wW = $wr.Right - $wr.Left; $wH = $wr.Bottom - $wr.Top
if ($wW -lt 500 -or $wH -lt 400) {
    Write-Output '{"status":"error","message":"wechat window too small","replies":[]}'
    exit
}
$sW = [int]($wW * 0.35)
...
for ($row = 0; $row -lt 10; $row++) {
    ...
    # Read chat title from foreground window or sidebar
    $chatTitle = [WxRA]::FgTitle()

    # Screenshot the chat area for AI vision
    $imgUrl = Get-ChatScreenshot
    if (-not $imgUrl) {
        $results += @{row=$row; status='screenshot_failed'}
        continue
    }
    ...
    $results += @{row=$row; reds=$reds; chat=$chatTitle; reply=$reply; status='sent'}
    Start-Sleep -Milliseconds 300
}
...
$output = @{
    status = 'success'
    total_scanned = 10
    replied_count = ($results | Where-Object { $_.status -eq 'sent' }).Count
    replies = $results
}
