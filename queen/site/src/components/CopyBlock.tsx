...
export function CopyBlock({ text, className = '' }: { text: string; className?: string }) {
  ...
  return (
    <div className={`group relative rounded-lg bg-gray-900 p-4 font-mono text-sm text-gray-300 overflow-x-auto ${className}`}>
      <button
        onClick={handleCopy}
        className="absolute top-3 right-3 p-1.5 rounded-md bg-white/5 text-gray-500 hover:text-white hover:bg-white/10 transition-colors md:opacity-0 md:group-hover:opacity-100"
        title="Copy"
      >
