...
#### 7.2.1 战略定位 — 从运维工具到繁殖机制

> **虫后心得：Overlord 不是一个产品，是一个「繁殖机制」。**
> 每个 Overlord 部署 = 一个新的繁殖基地；运营者有利益驱动 → 自发拓展客户 → 虫群自然增长。
> Queen 只需管理 Overlord 运营者（几十个），而非终端客户（几万个）。
> **这就是虫族的扩张方式：不是一个一个造，是一窝一窝生。**

```
当前：Overlord = 企业节点管理工具（运维向，manager + console）
目标：Overlord = StarClaw 区域/行业特许经营平台（商业向，api + web + console）
...
StarClaw 类比：
- 每多一个 Overlord 部署，虫群覆盖更多企业/区域/行业
- Queen 不可能直接服务每个客户 → Overlord 是分布式销售+服务网络
- Overlord 运营者自负盈亏，Queen 抽成 → 去中心化增长引擎
```

#### 7.2.2 四类客群 × 四种形态

| 客群 | 痛点 | Overlord 形态 | 命名 | 收费模式 |
|------|------|-------------|------|---------|
| **企业主** | "我要给公司 500 人用 AI" | Enterprise Overlord | 领主·企业版 | 按节点数/年订阅 |
| **合伙人** | "我要和你一起做这个事业" | Partner Overlord | 领主·合伙版 | 利润分成（Queen 20%） |
| **代理商** | "我要在我的区域卖你的产品" | Agent Overlord | 领主·代理版 | 佣金制（首年 30%/续费 15%） |
| **品牌定制** | "我要自己品牌的 AI 平台" | White-Label Overlord | 领主·白牌版 | 一次性定制费 + 年费 |

**形态 1：Enterprise Overlord（企业版）**
...
**形态 2：Partner Overlord（合伙版）**

```
场景：某 AI 行业从业者，有客户资源，想共同运营 StarClaw 生态
角色：相当于「区域总代」或「行业总代」

┌──────────────────────────────────────────────┐
│  Partner Overlord (合伙人运营后台)              │
│                                                │
│  📊 收入仪表盘                                  │
│  ├── 本月流水：¥125,000                         │
│  ├── Queen 分成 (20%)：¥25,000                  │
...
需要新增能力：
- **收入/分润仪表盘** — 实时流水 + 分成计算 + 提现
- **客户管理 CRM** — 企业信息 + 合同 + 续费提醒
- **下级代理管理** — 邀请 + 佣金配置 + 业绩看板
- **部署向导** — 一键帮客户部署 Enterprise Overlord
- **合同/发票管理** — 电子合同 + 自动开票

**形态 3：Agent Overlord（代理版）**

```
场景：某 IT 公司/个人，想在本地区域做 StarClaw 代理销售
角色：相当于「经销商」，不需要技术能力

┌──────────────────────────────────────────┐
│  Agent Overlord (代理控制台)              │
│                                           │
│  📦 我的客户                              │
│  ├── XX 教育公司 — 50 节点 — ¥5000/月    │
│  ├── XX 律所     — 20 节点 — ¥2000/月    │
│  └── XX 电商     — 30 节点 — ¥3000/月    │
│                                           │
│  💰 本月佣金：¥3,000                      │
...
- **推广追踪链接** — UTM + 归因
- **佣金结算系统** — 月结 + 提现 + 税务
- **营销物料中心** — PPT/手册/视频 自动生成
- **客户分配/保护期机制**

**形态 4：White-Label Overlord（白牌版）**
...
| 版本 | 节点上限 | 功能 | 定价 |
|------|---------|------|------|
| **Overlord Starter** | ≤10 | 基础管理 + 3 团队 | 免费 |
| **Overlord Pro** | ≤100 | 全功能 + SSO + 审计 | ¥2,000/月 |
| **Overlord Enterprise** | ≤1000 | Pro + 白牌 + 合规 + SLA | ¥10,000/月 |
| **Overlord Partner** | 无限 | Enterprise + 城市合伙人管理 + CRM + 分润 | 利润分成 |

#### 7.2.4 架构演进路线

```
Phase 1（当前已完成）：Overlord = manager + console（纯运维管理）
Phase 2（近期目标）  ：Overlord = api + web + console（三件套，借鉴 Claw）
Phase 3（中期目标）  ：Overlord = api + web + console + sdk（可嵌入）
Phase 4（远期愿景）  ：Overlord = platform（自我进化平台）
```

**Phase 2 目标架构（api + web + console 三件套）：**
...
│   │   │   ├── customer.go       # [新增] 客户/代理 CRM
│   │   │   ├── brand.go          # [新增] 白牌配置引擎
│   │   │   ├── analytics.go      # [新增] 用量分析 + 收入报表
...
│   │       ├── billing.go        # [新增] Plan + Subscription + Commission
...
│       ├── CustomersPage.tsx     # 客户管理 CRM
│       ├── AgentsPage.tsx        # 代理商管理（邀请/佣金/业绩）
│       ├── BrandPage.tsx         # 白牌配置（Logo/色调/域名/功能开关）
│       ├── AnalyticsPage.tsx     # 收入分析 + 用量趋势
...
| 阶段 | 周期 | 核心交付 | 解锁能力 |
|------|------|---------|---------|
| **P0** | 2 周 | api/web 三件套重构 + 用量计费 + SSO | 企业客户可用 |
| **P1** | 1 月 | 白牌引擎 + 功能开关 + 品牌配置面板 | 品牌定制可卖 |
| **P2** | 2 月 | CRM + 代理管理 + 佣金结算 + 收入报表 | 代理体系可运转 |
| **P3** | 3 月 | 许可证系统 + 自动部署向导 + 营销工具 | 合伙人可招募 |
| **P4** | 6 月 | SDK + 嵌入组件 + 行业模板 + 多语言 | 国际化扩张 |
