...
#### 7.2.1 战略定位 — 两个产品，各司其职

> **虫后心得：Overlord 是繁殖基地，Queen 是指挥中枢。**
> 每个 Overlord 部署 = 一个新的企业客户节点；Queen 管理所有合伙人和渠道。
> **Overlord 卖给客户赚钱，Queen 管理团队和渠道。边界清晰，各司其职。**

```
Overlord = 企业级 AI 节点管理产品（卖给客户的）
  • 部署在客户侧（私有云/内网）
  • 管理 Claw 节点、团队隔离、用量计费、SSO、合规审计
  • 可白牌定制（客户看到自己品牌）
  • 许可证分级：Community(免费) → Starter → Pro → Enterprise → White-Label

Queen Platform = 渠道管理中枢（自己团队和渠道用的）
  • 部署在 Queen 中央服务器
  • Queen Admin：创始人全局管理
  • Partner Hub：核心合伙人（内部团队）的 CRM + 佣金 + 期权 + 部署工具
  • City Portal：城市合伙人（外部渠道）的客户管理 + 佣金 + 营销物料

星际争霸类比：
- 领主（Overlord）提供「人口上限」→ 每多一个部署，虫群覆盖更多企业
- 脑虫（Cerebrate）是虫后的左右手 → 核心合伙人通过 Partner Hub 指挥渠道
- 虫后（Queen）统御全局 → Queen Admin 掌控所有数据和结算
```

#### 7.2.2 Overlord 产品体系（纯企业产品）

> **设计原则：** Overlord 只做一件事 — 企业级 AI 节点管理。渠道管理（CRM、佣金、合伙人管理）全部由 Queen Platform 承担。
> Overlord 是卖给客户赚钱的产品；Queen 是自己团队和渠道用的平台。

| 版本 | 节点上限 | 团队 | 核心功能 | 月付 | 年付（8折） |
|------|:-------:|:----:|---------|:----:|:----------:|
| **Community** | ≤10 | 1 | 开源核心，基础管理 | **免费** | — |
| **Starter** | ≤20 | 3 | + 用量统计 + 预算告警 | ¥499 | ¥399/月 |
| **Pro** ⭐ | ≤100 | 不限 | + SSO + 审计 + 高级分析 + 模型路由 | ¥1,999 | ¥1,599/月 |
| **Enterprise** | ≤500 | 不限 | + 合规面板 + SLA 99.9% + 专属技术支持 | ¥4,999 | ¥3,999/月 |
| **White-Label** | 不限 | 不限 | + 品牌全套替换 + 功能开关 + 自定义域名 | ¥9,999+ | 面议 |

> 详细功能矩阵、竞品对比、星能算力定价见 `business/pricing.md`

> Community 版开源 → 低门槛获客 → 企业用着好 → 升级付费版 → 典型开源商业化路径

**典型场景：Enterprise Overlord（企业版）**

```
场景：某制造业公司，500 员工，想把 AI 助手部署到全公司
当前痛点：用 ChatGPT 无法管控、数据外泄、无法审计

┌──────────────────────────────────────────┐
│  Enterprise Overlord (公司内网/私有云)     │
│                                           │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  │
│  │ 研发团队 │  │ 市场团队 │  │ HR 团队  │  │  ← 团队隔离（已有）
│  │ 20 Claw │  │ 10 Claw │  │ 5 Claw  │  │
│  │ 代码模型 │  │ 营销模型 │  │ 文档模型 │  │  ← 按团队配置不同模型
│  └─────────┘  └─────────┘  └─────────┘  │
│                                           │
│  🔒 数据不出内网  📊 用量审计  💰 成本控制  │
└──────────────────────────────────────────┘
...
需要新增能力：
- **用量计费仪表盘** — 按团队/按人/按模型的消耗统计与成本分摊
- **SSO 集成** — LDAP / SAML / OAuth 企业账号直登
- **数据合规面板** — 数据流向图 + 敏感词过滤 + 审计导出
- **模型路由策略** — 团队 A 只能用 DeepSeek，团队 B 可用 Claude
- **预算告警** — 团队月预算、自动降级/暂停

**典型场景：White-Label Overlord（白牌版）**

```
场景：某公司想推出自己品牌的 AI 平台，不想从零开发
需求："我要一个叫 XXX-AI 的平台，长我们的样子"

┌──────────────────────────────────────────────────┐
│  White-Label Overlord                             │
│                                                    │
│  🎨 品牌配置                                       │
│  ├── Logo: customer-logo.png                       │
│  ├── 品牌名: "智脑AI"                               │
│  ├── 主色调: #FF6B00                               │
│  ├── 域名: ai.customer-brand.com                   │
│  └── 备案号: 粤ICP备XXXXXXX号                       │
│                                                    │
│  🧬 功能开关                                       │
│  ├── ✅ AI对话  ✅ Agent模板  ✅ 文档分析             │
...
│  └── ✅ 企业SSO  ✅ 审计日志  ❌ 社区功能             │
│                                                    │
│  终端用户看到的是「智脑AI」，不知道底层是 StarClaw    │
└──────────────────────────────────────────────────┘
```

需要新增能力：
- **品牌配置引擎** — Logo/色调/名称/域名/favicon 全套替换
- **功能开关矩阵** — 按需启用/禁用能力模块
- **自定义 Agent 模板市场** — 客户自建 + 从 Queen 同步
- **自定义登录页/注册页/Landing Page**
- **多语言配置**

> **从 Overlord 中移除的能力（迁移到 Queen Platform）：**
> - ~~客户 CRM~~ → Queen Partner Hub
> - ~~代理/城市合伙人管理~~ → Queen Partner Hub
> - ~~佣金结算系统~~ → Queen 结算引擎
> - ~~营销物料中心~~ → Queen 内容管理
> - ~~收入/分润仪表盘~~ → Queen Admin + Partner Hub

#### 7.2.2.1 Queen Platform — 渠道管理中枢

> **设计原则：** 核心合伙人是内部员工，用公司内部系统；城市合伙人是外部渠道，用轻量门户。
> 二者的管理工具都部署在 Queen 中央平台，而非分散到各个 Overlord 实例。

```
┌─────────────────────────────────────────────────────────────────┐
│  Queen Platform（中央 SaaS）                                     │
│                                                                   │
│  ┌─────────────┐  ┌──────────────┐  ┌──────────────┐           │
│  │ Queen Admin  │  │ Partner Hub  │  │ City Portal  │           │
│  │ 虫后管理台    │  │ 核心合伙人    │  │ 城市合伙人    │           │
│  │              │  │ 工作台        │  │ 门户         │           │
│  │ • 全局大盘   │  │ • 客户 CRM   │  │ • 我的客户   │           │
│  │ • 合伙人管理 │  │ • 城市合伙人  │  │ • 我的佣金   │           │
...
│  └─────────────┘  └──────────────┘  └──────────────┘           │
│                                                                   │
│  同一 codebase，RBAC 控制角色视图                                  │
└─────────────────────────────────────────────────────────────────┘
```

**模块 1：Queen Admin（虫后管理台）** — 创始人/运维

| 功能 | 说明 |
|------|------|
| 全局大盘 | GMV、MRR、ARR、客户增长、续费率、渠道健康度 |
| 合伙人管理 | 核心合伙人绩效 + 城市合伙人审批/绩效 |
| 客户总览 | 所有企业客户、合同状态、健康评分 |
| 结算引擎 | 自动算佣金（双轨制）、生成账单、触发打款 |
| Overlord 监控 | 所有客户 Overlord 实例的健康状态 |
| 内容管理 | 营销物料、培训课程、销售话术库 |
| 系统配置 | 模型供应商接入、定价策略、许可证管理 |

**模块 2：Partner Hub（核心合伙人工作台）** — 内部团队
...
│  ├── 本月佣金：¥45,000（A轨）                      │
│  ├── 直签客户 5 家 / 月流水 ¥40,000                │
│  ├── 管理城市合伙人 3 人 / 签约客户 10 家           │
│  └── 期权已归属：1.5%（剩余 1.5% 待归属）          │
│                                                    │
│  👥 客户 CRM                                      │
│  ├── 线索 → 商机 → 签约 → 交付 → 续费              │
│  ├── XX 制造 — Pro 版 — ¥5,000/月 — 续费到 2026.12│
│  └── XX 律所 — Enterprise — ¥10,000/月 — 试用中    │
│                                                    │
│  🏙️ 我的城市合伙人                                │
│  ├── 张三 (杭州) — 3 客户 — ¥12,000/月            │
│  ├── 李四 (南京) — 2 客户 — ¥8,000/月             │
...
| 功能 | 说明 |
|------|------|
| 个人大盘 | 收入、KPI 达成率、管道漏斗 |
| 客户 CRM | 线索→商机→签约→交付→续费，全生命周期管理 |
| 城市合伙人管理 | 招募审批、培训进度、业绩跟踪、区域分配 |
| 佣金明细 | 双轨制分解：薪资 + 直签佣金 + 管理费，逐笔可查 |
| 期权归属 | Cliff 倒计时、已归属/未归属、当前估值参考 |
| 一键部署 | 帮客户一键部署 Overlord（Docker/K8s/云市场） |
| 物料库 | PPT、Demo 环境、报价单生成器、行业方案 |

**模块 3：City Portal（城市合伙人门户）** — 外部渠道
...
| 我的客户 | 客户列表、状态、续费提醒 |
| 我的佣金 | 月度明细、提现记录、税务凭证 |
| 营销工具 | 推广链接（UTM 归因）、报价单、Demo 环境 |
| 培训中心 | 产品知识、销售话术、行业方案（在线课程） |
| 业绩排名 | 全国城市合伙人排行榜 |
| 客服支持 | 提工单，核心合伙人或平台响应 |
...
| 角色 | 内部代号 | 性质 | 薪酬 |
|------|---------|------|------|
| **核心合伙人** | 脑虫 Cerebrate | 内部团队，有期权 | 双轨制（薪资+佣金+期权） |
| **城市合伙人** | 领主 Overlord | 外部渠道，平台直签 | 纯佣金制 |
| **企业客户** | 虫群 Brood | 最终付费方 | 订阅制 |
...
```
           ...
           ┌──────────┼──────────┐
           ▼          ▼          ▼
    核心合伙人 A  核心合伙人 B  核心合伙人 C    ← 脑虫（内部团队，期权+佣金）
        │            │           │
    ┌───┼───┐    ┌───┘      ┌───┼───┐
    ▼   ▼   ▼    ▼          ▼   ▼   ▼
   城市 城市 城市  城市       城市 城市 城市    ← 领主（外部渠道，与平台直签）
    │   │   │    │          │   │   │
    ▼   ▼   ▼    ▼          ▼   ▼   ▼
   企业 企业 企业  企业       企业 企业 企业    ← 虫群（企业客户，订阅制）
   ╱╲  ╱╲  ╱╲   ╱╲         ╱╲  ╱╲  ╱╲
  🦞🦞🦞🦞🦞🦞🦞🦞🦞        🦞🦞🦞🦞🦞🦞🦞   ← Claw 节点（开源免费）

法律关系：
  平台 ──雇佣──→ 核心合伙人（劳动合同 + 期权协议）
  平台 ──签约──→ 城市合伙人（合作协议，独立结算）
  核心合伙人管理城市合伙人，但不从城市合伙人佣金中抽成
```
...
企业客户月付 ¥10,000（示例）
    │
    ├── ① 节点订阅费（核心收入）    ¥8,000   ← 按节点数 × 单价
    ├── ② 星能消耗费（用量收入）    ¥1,500   ← AI 推理按量计费
    └── ③ 增值服务费（可选收入）    ¥500     ← 定制/培训/SLA
```

**场景 A：核心合伙人直签客户**

> 核心合伙人 = 公司内部团队（脑虫），佣金比例取决于薪酬轨道：A 轨（70%）或 B 轨（30%）
...
| 收入类型 | 核心合伙人 | 平台 (Queen) | 说明 |
|---------|:------:|:----------:|------|
| 节点订阅费 | **70%** | 30% | 核心合伙人代表平台签客户 |
| 星能消耗费 | **60%** | 40% | 算力成本较高，平台多留 |
| 增值服务费 | **85%** | 15% | 核心合伙人自己交付的服务 |

**B 轨（职业经理制，高薪低佣金）：**

| 收入类型 | 核心合伙人 | 平台 (Queen) | 说明 |
|---------|:------:|:----------:|------|
| 节点订阅费 | **30%** | 70% | 已有高薪，佣金低 |
| 星能消耗费 | **20%** | 80% | |
| 增值服务费 | **60%** | 40% | |

> 举例（A轨）：核心合伙人签 ¥10,000/月客户 → 薪资 ¥5,000 + 佣金 ~¥7,000 + 期权归属
> 举例（B轨）：核心合伙人签 ¥10,000/月客户 → 薪资 ¥20K-40K + 佣金 ~¥3,000 + 期权归属

**场景 B：城市合伙人签客户 — 平台直结 + 渠道管理服务费**
...
企业客户 ──付费──→ 平台（统一收款）
             ...
             ┌──────┼──────┐
             ▼          ▼          ▼
       城市合伙人 40%  平台 45%  核心合伙人 15%（渠道管理服务费）

法律关系：
  平台 ──签约──→ 城市合伙人（直接合同）
  平台 ──雇佣──→ 核心合伙人（内部团队，管理费作为薪酬组成部分）
```

| 收入类型 | 城市合伙人 | 核心合伙人（渠道管理服务费） | 平台 (Queen) |
|---------|:----:|:----------:|:----------:|
| 节点订阅费（**首年**） | **40%** | **15%** | 45% |
| 节点订阅费（**续费**） | **25%** | **8%** | 67% |
| 星能消耗费 | **30%** | **10%** | 60% |
...
> 设计逻辑：首年城市合伙人拿多（激励拓新）；续费平台多留（产品驱动留存）；核心合伙人作为内部团队收渠道管理费（平台作为薪酬组成部分支付，不从城市合伙人佣金中抽取）

> 举例（首年，A轨）：城市合伙人签 ¥10,000/月客户 → 城市合伙人 ~¥4,000 + 核心合伙人 ~¥1,500（管理费）+ 平台 ~¥4,500

**阶梯激励 — 核心合伙人量大优享：**

| 核心合伙人月度总流水 | 核心合伙人分成 | 平台分成 | 等级 |
|:-------------|:--------:|:------:|------|
| < ¥50,000 | 70% | 30% | 🥉 铜级领主 |
| ¥50,000 - ¥200,000 | 75% | 25% | 🥈 银级领主 |
| ¥200,000 - ¥500,000 | 78% | 22% | 🥇 金级领主 |
| > ¥500,000 | 80% | 20% | 💎 钻石领主 |
...
| 打款时间 | 每月 10 号前打款 |
| 最低提现 | ¥500 |
| 税务 | 平台依法代扣代缴个人所得税，并提供佣金/服务费明细 |
| 客户保护期 | 城市合伙人签约客户 12 个月内不可被其他城市合伙人抢走 |
| 退款处理 | 客户退款则等比扣回已发佣金 |

#### 7.2.4 架构演进路线

```
Phase 1（当前已完成）：Overlord = manager + console（纯运维管理）
Phase 2（近期目标）  ：Overlord = api + web + console（企业三件套）
                      Queen += Partner Hub + City Portal（渠道管理）
Phase 3（中期目标）  ：Overlord += sdk（可嵌入）
                      Queen += 结算引擎 + Ops Center
Phase 4（远期愿景）  ：Overlord = platform（自我进化）
                      Queen = 生态编排器
```

**Phase 2 目标架构 — Overlord（纯企业产品）：**

```
overlord/
├── api/                          # Go 后端 (:8095)  ← 原 manager/ 重构扩展
│   ├── cmd/server/main.go
│   ├── internal/
│   │   ├── handler/
...
│   │   │   └── compliance.go     # [新增] 数据合规 + 审计导出
│   │   ├── middleware/
│   │   │   ├── auth.go           # [已有] AdminAuth + RBAC
│   │   │   └── license.go        # [新增] 许可证校验
│   │   └── model/
│   │       ├── brood.go          # [已有] ClawNode + TaskAssignment + AuditLog
│   │       ├── team.go           # [已有] Team + AdminUser + RolePermissions
│   │       ├── billing.go        # [新增] Plan + Subscription + Usage
│   │       └── brand.go          # [新增] BrandConfig + FeatureToggle
│   └── configs/
│       └── features.yaml         # 功能开关矩阵
│
├── web/                          # [新增] 终端用户 AI 工作台 (:3096)
│   └── src/                      # 面向企业员工的前端（可白牌）
│       ├── pages/
│       │   ├── ChatPage.tsx      # AI 对话（代理到管辖 Claw）
│       │   ├── AgentsPage.tsx    # Agent 模板市场（从 Queen 同步 + 本地自建）
│       │   ├── ToolsPage.tsx     # 工具集合
│       │   └── ProfilePage.tsx   # 个人用量/设置
│       └── lib/
│           └── brand.ts          # 动态品牌配置加载（Logo/色调/名称）
│
├── console/                      # [已有] 管理控制台 (:3095)，扩展新页面
│   └── src/pages/                # 新增：
│       ├── BillingPage.tsx       # 计费管理（按团队/按人/按模型）
│       ├── BrandPage.tsx         # 白牌配置（Logo/色调/域名/功能开关）
│       ├── AnalyticsPage.tsx     # 用量趋势 + 成本分析
│       └── CompliancePage.tsx    # 合规中心（审计导出/数据流向/敏感词）
│
└── sdk/                          # [Phase 3] 嵌入式 SDK
    ├── js/                       # 前端嵌入组件（Chat Widget / Agent Picker）
    └── go/                       # 后端 SDK（Claw 能力快速集成）
```

> **对比旧版移除的文件：** `customer.go`（CRM）、`AgentsPage.tsx`（代理管理）、`CustomersPage.tsx`（客户管理）→ 全部迁移到 Queen Platform

**Phase 2 目标架构 — Queen Platform（渠道管理）：**
...
│   │   ├── handler/
│   │   │   ├── ...               # [已有] 现有 Queen 功能
│   │   │   ├── admin/            # [新增] Queen Admin 模块
│   │   │   │   ├── dashboard.go  # 全局大盘（GMV/MRR/ARR）
│   │   │   │   ├── partners.go   # 所有合伙人管理
│   │   │   │   ├── clients.go    # 所有企业客户总览
│   │   │   │   ├── settlement.go # 结算引擎（自动算佣金+打款）
│   │   │   │   ├── overlords.go  # 所有 Overlord 实例监控
│   │   │   │   └── content.go    # 营销物料/培训内容管理
│   │   │   ├── partner/          # [新增] Partner Hub 模块
│   │   │   │   ├── dashboard.go  # 个人大盘（收入/KPI/漏斗）
│   │   │   │   ├── crm.go        # 客户 CRM（全生命周期）
│   │   │   │   ├── city.go       # 城市合伙人管理（招募/培训/绩效）
│   │   │   │   ├── commission.go # 佣金明细（双轨制分解）
│   │   │   │   ├── equity.go     # 期权归属跟踪
│   │   │   │   └── deploy.go     # 一键部署 Overlord
│   │   │   └── city/             # [新增] City Portal 模块
│   │   │       ├── dashboard.go  # 我的大盘
│   │   │       ├── clients.go    # 我的客户列表
│   │   │       ├── commission.go # 我的佣金（提现/税务凭证）
│   │   │       └── materials.go  # 营销物料 + 培训课程
│   │   ├── middleware/
│   │   │   └── role.go           # [新增] RBAC: admin / partner / city
│   │   └── model/
│   │       ├── partner.go        # [新增] CorePartner + CityPartner
│   │       ├── customer.go       # [新增] Customer + Contract + Renewal
│   │       ├── commission.go     # [新增] Commission + Settlement + Payout
│   │       └── content.go        # [新增] Material + Course + Template
│   └── ...
│
├── web/                          # [新增] Queen 前端（三个 SPA 共享组件库）
│   ├── admin/                    # Queen Admin SPA
│   │   └── src/pages/
│   │       ├── DashboardPage.tsx # 全局大盘
│   │       ├── PartnersPage.tsx  # 合伙人管理
│   │       ├── ClientsPage.tsx   # 客户总览
│   │       ├── SettlementPage.tsx# 结算管理
│   │       └── ContentPage.tsx   # 内容管理
│   ├── partner/                  # Partner Hub SPA
│   │   └── src/pages/
│   │       ├── DashboardPage.tsx # 个人大盘
│   │       ├── CRMPage.tsx       # 客户 CRM
│   │       ├── CityPage.tsx      # 城市合伙人管理
│   │       ├── CommissionPage.tsx# 佣金明细
│   │       ├── EquityPage.tsx    # 期权归属
│   │       └── DeployPage.tsx    # 一键部署
│   └── city/                     # City Portal SPA
│       └── src/pages/
│           ├── DashboardPage.tsx # 我的大盘
│           ├── ClientsPage.tsx   # 我的客户
│           ├── CommissionPage.tsx# 我的佣金
│           └── MaterialsPage.tsx # 营销物料 + 培训
```

**web/ 与 Claw web/ 的关系：**
- Claw web 是面向个人用户的 AI 助手界面
- Overlord web 是面向企业员工的 AI 工作台，共享 Claw 的 AI 能力，但增加企业管理层
- Overlord web 支持白牌定制（Logo/色调/名称/域名），终端用户可完全看不到 StarClaw 品牌
- Queen web 是面向内部团队和渠道合伙人的管理平台，不面向终端用户

#### 7.2.5 实施路线图

| 阶段 | 周期 | 核心交付 | 解锁能力 |
|------|------|---------|---------|
| **P0** | 2 周 | Overlord api/web 三件套 + 用量计费 + SSO | 企业客户可卖 |
| **P1** | 1 月 | Queen City Portal（佣金查看 + 客户管理 + 营销工具） | 城市合伙人可招募 |
| **P2** | 2 月 | Queen Partner Hub（CRM + 佣金 + 城市合伙人管理 + 部署向导） | 核心合伙人有工具 |
| **P3** | 3 月 | Overlord 白牌引擎 + 许可证系统 | 白牌客户可卖 |
| **P4** | 4 月 | Queen Admin（结算引擎 + 全局大盘 + Overlord 监控） | 自动化结算+运维 |
| **P5** | 6 月 | Overlord SDK + 行业模板 + 多语言 | 国际化扩张 |

**P0 详细拆解（2 周冲刺）：**

| 天 | 任务 | 交付物 |
|----|------|--------|
| D1-D2 | `manager/` → `api/` 重命名 + 路由不变 | 零停机迁移 |
| D3-D4 | `web/` 基础搭建（Vite+React+Tailwind）+ ChatPage | 终端用户可对话 |
| D5-D6 | billing handler + model（用量统计 + 团队账单） | 管理员看用量 |
| D7-D8 | SSO handler（OAuth2 通用 + LDAP 适配） | 企业员工免注册 |
| D9-D10 | console BillingPage + AnalyticsPage | 管理控制台新页面 |
| D11-D12 | web AgentsPage + ToolsPage + 品牌色调基础 | 终端用户完整体验 |
| D13-D14 | 联调测试 + Docker Compose + 文档 | 可交付 MVP |
...
### 15.1 ⚔️ 军事 — 虫族战士 vs 大龙虾（OpenClaw）

StarClaw 的核心竞争力来自**虫族战士（Warrior Claw）**——在 AI Agent 能力竞技中击败对手。

**战场：** 与其他开源 AI Agent 平台（大龙虾泛指所有竞品）的直接较量。
...
- [ ] **P0** `manager/` → `api/` 重构 + `web/` 终端用户 AI 工作台（借鉴 Claw api/web 架构）
- [ ] **P0** billing handler — 用量计费仪表盘（按团队/按人/按模型统计 + 预算告警）
- [ ] **P0** SSO handler — LDAP / SAML / OAuth 企业账号直登
- [ ] **P1** brand handler — 白牌配置引擎（Logo/色调/名称/域名/favicon 全套替换）
- [ ] **P1** features.yaml — 功能开关矩阵（按需启用/禁用能力模块）
- [ ] **P1** console BrandPage — 品牌配置 + 功能开关管理界面
- [ ] **P2** customer handler — 客户/代理 CRM（企业信息 + 合同 + 续费提醒）
- [ ] **P2** commission model — 代理佣金结算系统（月结 + 提现 + 税务）
- [ ] **P2** console AnalyticsPage — 收入分析 + 用量趋势 + 分润报表
- [ ] **P3** license middleware — 许可证体系（Starter/Pro/Enterprise/Partner 四级）
- [ ] **P3** 自动部署向导 — 一键帮客户部署 Enterprise Overlord
- [ ] **P3** 营销工具 — 推广追踪链接 + 产品手册 + Demo 环境 + 报价单生成器
- [ ] **P4** overlord/sdk/ — JS 嵌入组件（Chat Widget / Agent Picker）+ Go 后端 SDK
- [ ] **P4** 行业模板 — 教育/法律/电商/制造等垂直行业预配置
- [ ] **P4** 多语言 — i18n 国际化（中/英/日）
- [ ] compliance handler — 数据合规面板（数据流向图 + 敏感词过滤 + 审计导出）
