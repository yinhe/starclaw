...
import { parseScriptMarkdown, SAMPLE_SCRIPT_MD } from './scriptParser'
...
export default function ScriptImporterModal({ open, adType, onClose, onImport }: Props) {
          ...
          <div className="flex flex-col gap-2">
            ...
            {!preview ? (
              ...
              <div className="flex-1 min-h-[420px] bg-gray-950 border border-gray-800 rounded-lg p-3 overflow-y-auto">
                <div className="text-sm text-gray-100 font-medium">{preview.label}</div>
                <div className="text-[11px] text-gray-500 mt-0.5">{preview.description}</div>
                <div className="mt-2 text-[10px] text-gray-500 flex items-center gap-3">
                  <span>{preview.scenes?.length || 0} 镜</span>
                  <span>{preview.duration || 0}s</span>
                  {preview.video_resolution && <span>{preview.video_resolution}</span>}
                  {preview.video_ratio && <span>{preview.video_ratio}</span>}
                </div>
                <div className="mt-3 space-y-1.5">
                  {(preview.scenes || []).map(s => (
                    <div key={s.id} className="px-2 py-1.5 rounded bg-gray-900 border border-gray-800">
                      <div className="text-[11px] text-gray-200 flex items-center justify-between">
                        <span className="font-medium">{s.id} · {s.label}</span>
                        <span className="text-[10px] text-gray-500">{s.duration}s</span>
                      </div>
                      {s.prompt && (
                        <div className="text-[10px] text-gray-500 mt-0.5 line-clamp-2">{s.prompt}</div>
                      )}
                      {s.storyboard_url && (
                        <div className="text-[10px] text-cyan-500 mt-0.5 truncate">📷 {s.storyboard_url}</div>
                      )}
                    </div>
                  ))}
                </div>
        ...
        <div className="px-5 py-3 border-t border-gray-800 flex items-center justify-between bg-gray-950">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={submit}
              disabled={!preview || (preview.scenes?.length || 0) === 0}
