...
class _BillingScreenState extends State<BillingScreen> {
        ...
        return StatefulBuilder(
          builder: (ctx, setSheetState) {
            Future<void> pay(String method) async {
              if (paying || pkgId.isEmpty) return;
              setSheetState(() => paying = true);
              try {
                final res = await ApiService().createOrder(pkgId, method);
                final data = res.data;
                ...
                if (payUrl.isNotEmpty) {
                  ...
                  // Poll order status after user returns
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(
                      context,
                    ).showSnackBar(const SnackBar(content: Text('获取支付链接失败')));
                  }
                }
              } catch (e) {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        '创建订单失败: ${e.toString().length > 60 ? e.toString().substring(0, 60) : e}',
                      ),
                    ),
                  );
                }
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: paying ? null : () => pay('alipay'),
                      icon: paying
                          ? const SizedBox(
                              width: 18,
                              height: 18,
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: paying ? null : () => pay('wechatpay'),
                      icon: const Icon(
                        Icons.chat_rounded,
  ...
  void _pollOrderStatus(String orderNo) {
    int attempts = 0;
    ...
    Timer.periodic(const Duration(seconds: 3), (timer) async {
      ...
      try {
        final res = await ApiService().queryOrderStatus(orderNo);
