# Agent 六元组架构 (Agent Hexad)

> 一个 Agent = 基因 + 技能 + 本能 + MCP 外接 + 工作流 + 记忆
>
> Gene + Skill + Instinct + MCP + Workflow + Memory

## 一、设计原则

1. **六元组完备** — 任何 Agent（内置/市场/自建）都由且仅由这 6 个组件构成
2. **MCP 唯一外接** — 所有外部工具通过 MCP 协议接入，废弃 plugin JSON 文件
3. **安装即完整** — 从市场安装一个 Agent = 一次性部署全部 6 个组件
