...
// sttAttempt represents a single STT provider attempt configuration.
...
// findSTTAttempts returns all available STT provider configurations for a user.
func (h *MultimodalHandler) findSTTAttempts(userID string) []sttAttempt {
	var attempts []sttAttempt
	priority := []string{"qwen", "openai", "deepseek"}
	...
	for _, prov := range priority {
		...
		if err := h.db.Where("user_id = ? AND provider = ? AND is_enabled = ? AND api_key != '' AND api_key != 'claw-identity' AND deleted_at IS NULL", userID, prov, true).First(&cfg).Error; err == nil {
			...
			attempts = append(attempts, sttAttempt{Provider: prov, APIKey: cfg.APIKey, BaseURL: base, Model: sttModelForProvider(prov)})
		}
	}

	// Fallback: any enabled provider with a real API key
	var cfg model.ModelConfig
	if err := h.db.Where("user_id = ? AND is_enabled = ? AND api_key != '' AND api_key != 'claw-identity' AND provider != 'star-ai' AND deleted_at IS NULL", userID, true).First(&cfg).Error; err == nil {
		// Only add if not already present
		dup := false
		for _, a := range attempts {
			if a.Provider == cfg.Provider {
				dup = true
				break
			}
		}
		if !dup {
			...
			attempts = append(attempts, sttAttempt{Provider: cfg.Provider, APIKey: cfg.APIKey, BaseURL: base, Model: sttModelForProvider(cfg.Provider)})
		}
	}

	// Last resort: config file
	if len(attempts) == 0 && h.cfg.OpenAI.APIKey != "" {
		base := h.cfg.OpenAI.BaseURL
		if base == "" {
			base = "https://api.openai.com/v1"
		}
		attempts = append(attempts, sttAttempt{Provider: "openai", APIKey: h.cfg.OpenAI.APIKey, BaseURL: base, Model: "whisper-1"})
	}

	return attempts
}

// transcribeAudio sends a WAV buffer to the Whisper API and returns the transcribed text.
func (h *MultimodalHandler) transcribeAudio(ctx context.Context, userID, filename string, wavData []byte) (string, error) {
	prov, apiKey, baseURL, found := h.findSTTProvider(userID)
	if !found {
		return "", fmt.Errorf("no STT provider configured")
	}
	sttModel := sttModelForProvider(prov)
	...
	fmt.Fprintf(w, "--%s\r\n", boundary)
	fmt.Fprintf(w, "Content-Disposition: form-data; name=\"file\"; filename=\"%s\"\r\n", filename)
	fmt.Fprintf(w, "Content-Type: audio/wav\r\n\r\n")
	w.Write(wavData)
	fmt.Fprintf(w, "\r\n")
