...
func (h *TemplateHandler) CommunityList(c *gin.Context) {
	...
	for _, a := range agents {
		...
		cfgMap := map[string]string{"system_prompt": a.SystemPrompt, "tools": a.Tools}
		cfgBytes, _ := json.Marshal(cfgMap)
		items = append(items, gin.H{
			"id": a.ID, "name": a.Name, "description": a.Description,
			"icon": "", "tags": "[]", "config": string(cfgBytes),
...
func (h *TemplateHandler) CommunityGet(c *gin.Context) {
	...
	if err := h.db.Where("id = ?", id).First(&agent).Error; err == nil {
		cfgMap := map[string]string{"system_prompt": agent.SystemPrompt, "tools": agent.Tools}
		cfgBytes, _ := json.Marshal(cfgMap)
		c.JSON(http.StatusOK, gin.H{
			"item": gin.H{
