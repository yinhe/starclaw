...
// ── Marketplace Purchase (本地购买记录 — 社区市场付费 Agent) ──

// MarketplacePurchase tracks a user's purchase of a paid community template.
// Stored locally in Claw's SQLite DB. Star energy is deducted via Queen billing.
type MarketplacePurchase struct {
	...
	AgentID      string     `json:"agent_id" gorm:"type:varchar(36)"`     // installed agent ID
	PricingType  string     `json:"pricing_type" gorm:"type:varchar(20)"` // one_time, subscription
	PriceCents   int        `json:"price_cents" gorm:"default:0"`         // amount paid in cents (分)
	Period       string     `json:"period" gorm:"type:varchar(20)"`       // month, quarter, year (subscription only)
	Currency     string     `json:"currency" gorm:"type:varchar(10);default:CNY"`
	Status       string     `json:"status" gorm:"type:varchar(20);default:active;index"` // active, expired, cancelled, refunded
	PurchasedAt  time.Time  `json:"purchased_at"`
	ExpiresAt    *time.Time `json:"expires_at"` // nil for one_time (永久)
	CreatedAt    time.Time  `json:"created_at"`
	UpdatedAt    time.Time  `json:"updated_at"`
}
...
// IsActive returns true if the purchase is currently valid (not expired, not cancelled).
func (p *MarketplacePurchase) IsActive() bool {
	if p.Status != "active" {
		return false
	}
